...
public class DataBaseTest
{
    ...
    public void CanStoreAndRetrieveJsonData()
    {
        ...
        var jsonData =JsonConvert.DeserializeObject<WeatherData>(jsonContent);

        // Act
        context.WeatherData.Add(jsonData);
        context.SaveChanges();

        var retrievedJsonData = context.WeatherData.LastOrDefaultAsync(x => x.Id == jsonData.Id);

        // Assert
        Assert.NotNull(retrievedJsonData);
        Assert.Equal(jsonContent,JsonConvert.SerializeObject(jsonContent) );
    }
